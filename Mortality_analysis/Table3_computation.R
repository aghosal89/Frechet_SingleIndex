...
# For Global Frechet
frechet_Rsquared(Y=quant_all, X=X_ctr, tt=qSup, model="GF")

# For Local Frechet: HDI
h_hdi<- read.csv("LF_HDI_BW.csv", header = T)[,2]
frechet_Rsquared(Y=quant_all, X=(X_ctr[,"HDI"]), tt= qSup, h=h_hdi, model="LF")

# For Local Frechet: Healthcare Expenditure as percentage of GDP
h_hce<- read.csv("LF_HCE_BW.csv", header = T)[,2]
frechet_Rsquared(Y=quant_all, X=(X_ctr[,"HCE"]), tt= qSup, h=h_hce, model="LF")

# For Local Frechet: GDP
h_gdpc<- read.csv("LF_GDPC_BW.csv", header = T)[,2]
frechet_Rsquared(Y=quant_all, X=(X_ctr[,"GDPC"]), tt= qSup, h=h_gdpc, model="LF")

# For Local Frechet: Infant Mortality
h_im<- read.csv("LF_IM_BW.csv", header = T)[,2]
frechet_Rsquared(Y=quant_all, X=(X_ctr[,"IM"]), tt= qSup, h=h_im, model="LF")


# For Local Frechet: CO2em
h_co2e<- read.csv("LF_CO2E_BW.csv", header = T)[,2]
frechet_Rsquared(Y=quant_all, X=(X_ctr[,"CO2E"]), tt= qSup, h=h_co2e, model="LF")
...
frechet_Rsquared(Y=quant_all, X=(X_ctr%*%theta), tt= qSup, h=h_fsi, model="LF")
...
df_mspe_folds <- rbind(data.frame(Model= 'GF',MSPE =gf_folds[,2]), 
                       data.frame(Model= 'LF(HDI)', MSPE=lf_hdi_folds[,2]), 
                       data.frame(Model= 'LF(HCE)',MSPE=lf_hce_folds[,2]),
                       data.frame(Model='LF(GDPC)', MSPE=lf_gdpc_folds[,2]),
