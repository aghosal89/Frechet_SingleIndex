...
%         optRet = flag for returning optimizer info (1 = yes [default], 0 = no)
...
function fsi_fit = get_sphere_fit_FSI(Y, x, h, xout, theta_init, optRet)
...
%% Check Inputs
    ...
    elseif(length(theta_init) == 1)
        ...
        theta_init = cell2mat(arrayfun(@(j) polar2cartGen(eta_init(:, j)), 1:q, 'UniformOutput', false));
    end
