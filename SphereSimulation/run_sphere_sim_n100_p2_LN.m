...
s_dt = 433287;  % seed for generating random data (different values for different sims/batches)
...
optType = 'SQP';
q = 10; % # of random starting points
eta_init = pi*rand(p - 1, q) - pi/2;
thetaInit = cell2mat(arrayfun(@(j) polar2cart(eta_init(:, j)), 1:q, 'UniformOutput', false));

optns = struct('Ret', 1, 'maxIter', 30, 'thetaInit', thetaInit, 'thetaChoose', 2, 'Algorithm', optType); % max of 30 iterations per starting value
