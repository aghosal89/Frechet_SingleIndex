function [WnGrad, WnHess, WnCostAlt] = getWnGradHess(eta, Y, x, h)
...
for i = 1:n
    ...
    % Get derivative of normalized local linear fit at x(i, :)    
        ...
        ttil(:, :, i) = t*b; % derivative of normalized local linear fit at x(i, :)
    ...
    % Get Hessian of normalized local linear fit at x(i, :)
        ...
        for k = 1:p
            ...
            Ttil(:, :, k, i) = squeeze(T(:, :, k))*b + t*tmp;
            
        end
         % Hessian of normalized local linear fit at x(:, i)
end

% Compute grad and Hessian w.r.t. theta first

grad1 = 0; hess1 = 0;

for i = 1:n
    
    tti = squeeze(ttil(:, :, i)); Yi = Y(:, i); Tti = squeeze(Ttil(:, :, :, i));
    
    ip = Yi'*Ytil(:, i);
    grad1 = grad1 - 2*acos(ip)*(1 - ip^2)^(-1/2)*tti*Yi;
    
    tmp = cell2mat(arrayfun(@(k) squeeze(Tti(:, :, k)*Yi), 1:p, 'UniformOutput', false));
    hess1 = hess1 - 2*(tti*Yi*Yi'*tti')*(1 - ip^2)^(-1)*(1 + acos(ip)*(1 - ip^2)^(-1/2)) + acos(ip)*(1 - ip^2)^(-1/2)*tmp;
    
end
hess1 = (hess1 + hess1')/2; % symmetrize
...
WnGrad = J*grad1/n;
WnHess = J*hess1*J'/n;
WnCostAlt = mean((arrayfun(@(g) acos(Y(:, g)'*Ytil(:, g)), 1:n)).^2); % prediction error on sphere
