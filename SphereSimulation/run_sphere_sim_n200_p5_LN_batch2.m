...
numWk = 30; % if usePar = 1, how many workers should be used.
...
nsim = 60;    % number of simulations
n = 200;        % number of observations
p = 5;         % dimension of index parameter
noise = 'Low'; % High or low noise setting
h = 0.15:0.1:0.45; % bandwidth grid

% Define random seeds
s_pm = 49156;  % seed for generating true theta parameter (should be same across all sims/batches sharing same p)
s_dt = 274618;  % seed for generating random data (different values for different sims/batches)
...
optType = 'SQP';
q = 50; % # of random starting points
eta_init = pi*rand(p - 1, q) - pi/2;
thetaInit = cell2mat(arrayfun(@(j) polar2cart(eta_init(:, j)), 1:q, 'UniformOutput', false));

optns = struct('Ret', 1, 'maxIter', 50, 'thetaInit', thetaInit, 'thetaChoose', 3, 'Algorithm', optType); % max of 50 iterations per starting value
...
fnm = strcat('Sphere_results_n', num2str(n), '_nsim', num2str(nsim), '_p', num2str(p), '_noise', noise, 'batch2.mat');
